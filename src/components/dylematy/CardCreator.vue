<script setup lang="ts">
  ...
  const actualDecks = ref<ListElement[]>(userStore.user.decks.map(convertDecksToListElement));
  ...
  const addCardBtn = computed(() => ({
    ...
    isOrange: true,
    action: addCard,
    disabled: !actualDecks.value.filter(deck => deck.isSelected).length,
  }));
...
<template>
  <div class="cardCreator creamCard">
    ...
    <WhiteCard v-if="!addMany" :header="$t('dylematy.createdCard', { type: translatedCardType })">
      <div class="card">
        {{ newCard.text }}
      </div>
    </WhiteCard>
