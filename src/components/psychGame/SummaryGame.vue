<script setup lang="ts">
  ...
  const currentUser =
    props.users.find(user => user.userId === useUserStore().user.userId) ?? new User();

  const votesReceived = [...currentUser.votesReceived]
    .sort((a, b) => b.voteCount - a.voteCount)
    .map(
      vote => `${props.users.find(u => u.userId === vote.voterId)?.username} (${vote.voteCount})`
    )
    .join(', ');
  ...
  const totalVotesReceived = currentUser.votesReceived.reduce((s, v) => s + v.voteCount, 0);
  ...
  const cards = [
    ...
    {
      header: t('psych.summary.inGameYourOwnAnswerWasExperiencedTimesIncludingYouTimes', {
        x: 4,
        x2: 2,
      }),
    },
